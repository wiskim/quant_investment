...
import pandas as pd
df = pd.read_csv(os.path.join(data_path, 'dataguide_price.csv'), index_col=[0, 1, 2, 3, 4, 5])
df = df.stack().reset_index()
df.columns = ['stock_cd', 'stock_nm', 'kind', 'item', 'price_div', 'freq', 'date', 'price']
df['price_div'] = df['price_div'].replace({'시가(원)':'Open', '저가(원)':'Low', 
                                           '고가(원)':'High', '종가(원)':'Close', 
                                           '거래대금(원)':'Volume'})
